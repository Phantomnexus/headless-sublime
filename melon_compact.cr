...
class MelonHarvester
  ...
  def start
    ...
    # FIX: round UP to the south boundary of the current plot, not down.
    # The old code rounded down (north), making go_front_line move to the
    # plot ahead instead of the one the bot is standing in.
    remainder = (cur_z + 1) % PLOT_HEIGHT
    @current_plot = remainder == 0 ? cur_z : cur_z + (PLOT_HEIGHT - remainder)
  ...
  private def harvest_line
    ...
    # FIX: mirrors the stuck handler — look down at the entrance melon,
    # break it, then look back up before walking. Clears any melon blocking
    # the row entrance before move_to starts.
    bot.stop_digging
  ...
  # Walks to (target_x, z) on a fixed yaw while holding attack.
  # A background fiber keeps swapping to a fresh axe every 4 ticks so
  # durability is managed without breaking the tool completely.
  # If movement stalls (MovementStuck), looks steeply down, chews through
  # the blocker, then retries. Abandons the row after MAX_STUCK_RETRIES
  # consecutive stalls.
  private def walk_attacking_to(target_x : Int32, z : Int32, yaw : Float32)
    ...
    begin
      ...
      while bot.connected?
        x = bot.x.floor.to_i
        # FIX: going_east recalculated each iteration so an overshoot during
        # go_front_line doesn't permanently flip the direction and skip the row.
        going_east = target_x > x
  ...
  # attacking so any melons in the path get cleared.
  ...
  private def go_front_line
    ...
    # FIX: attack while moving so any melons that grew between rows get cleared.
    bot.look = Rosegold::Look.new((90 + (1 - @dir) * 180).to_f32, PITCH_STUCK)
    bot.start_digging
